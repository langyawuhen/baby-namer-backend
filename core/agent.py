...
async def generate_names(name_info: NameIn) -> NamesResultSchema | None:
    prompt = f"用户的姓氏是：{name_info.surname}，用户的性别为：{name_info.gender}，用户的字数要求为：{name_info.length}，用户的其他要求为：{name_info.other}，这些名字不要：{'、'.join(name_info.exclude)}，请为用户生成5个符合要求的姓名。"
    messages = [
        SystemMessage(content=system_prompt),
    ...
    if result is None:
        json_instruction = (
            '请仅输出一个 JSON 对象，不要其他说明。格式为：{"names": [{"name": "姓名", "reference": "出处", "moral": "寓意"}, ...]}，共5个姓名。'
        )
        fallback_messages = [
            SystemMessage(content=system_prompt),
            HumanMessage(content=prompt + "\n\n" + json_instruction),
